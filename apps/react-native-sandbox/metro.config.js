...
const monoRepoFolders = workspacePackage.workspaces.map(pkg =>
  pkg.substring(0, pkg.search(RegExp('\\/\\*'))),
);

function findSharedPackages(sharedPackagesFolder) {
...
const allRepoPackages = findSharedPackages(monoRepoFolders);
